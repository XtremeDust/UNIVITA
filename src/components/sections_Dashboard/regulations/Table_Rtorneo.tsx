...
const handleConfirmDelete = async () => {
  ...
  const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api';
    const deletePromise = async () => {
        const res = await fetch(`${API_URL}/tournament-regulations/${regToDelete.id}`, { 
            method: 'DELETE',
    ...
    toast.promise(deletePromise(), {
        loading: 'Eliminando reglamento...',
        success: '¡Reglamento eliminado correctamente!',
...
const handleEditClick = (regulation: ApiTournamentRegulation) => {
  ...
  setEditingReg(regulation);
    ...
    const filteredRegs = tournamentRegs.filter((item) => {
    const term = searchTerm.toLowerCase();
    return (
        item.torneo.nombre.toLowerCase().includes(term) ||
        item.reglamento.titulo.toLowerCase().includes(term) ||
        (item.reglamento.creator && item.reglamento.creator.toLowerCase().includes(term))
    );
    });
