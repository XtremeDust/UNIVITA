...
export default function Modal_addRegulation({ 
  ...
  const handleSave = async () => {
    ...
    const saveRegulationPromise = async () => {
        ...
        if (!res.ok) {
            ...
            throw new Error(errData.message || `Error al guardar el reglamento`);
        }
        return res;
    };

    toast.promise(saveRegulationPromise(), {
        loading: isEditMode ? 'Actualizando documento...' : 'Subiendo reglamento...',
        success: '¡Reglamento guardado correctamente!',
  ...
  return (
    <Modal state={state}>
      <ContainModal className="w-full max-w-2xl bg-white rounded-xl shadow-2xl flex flex-col max-h-[90vh] sm:max-h-[85vh]  overflow-y-auto text-black">
