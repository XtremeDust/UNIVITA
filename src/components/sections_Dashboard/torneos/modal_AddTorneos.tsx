...
export default function Modal_addtorneos({ state, onClose, onTournamentCreated, tournamentToEdit }: ModalProps) {
    ...
    const validateForm = () => {
        ...
        if (!isEditMode && !formData.img) {
            ...
            isValid = false;
        } else if (isEditMode && !formData.img && !formData.previewUrl) {
            newErrors.img = 'La imagen del torneo es obligatoria.';
            isValid = false;
        }
    ...
    const handleSave = async () => {
        ...
        const saveTournamentPromise = async () => {
            ...
            if (!res.ok) {
                ...
                if (res.status === 422 && errorData.errors) {
                    const backendErrors = { name: '', descrip: '', state: '', dates: '', img: '' };
                    if (errorData.errors.name) backendErrors.name = errorData.errors.name[0];
                    if (errorData.errors.descrip) backendErrors.descrip = errorData.errors.descrip[0];
                    if (errorData.errors.state) backendErrors.state = errorData.errors.state[0];
                    if (errorData.errors.init_date || errorData.errors.end_date) {
                        backendErrors.dates = errorData.errors.init_date?.[0] || errorData.errors.end_date?.[0];
                    }
                    if (errorData.errors.img) backendErrors.img = errorData.errors.img[0];
                    setErrors(prev => ({ ...prev, ...backendErrors }));
                    throw new Error("Hay campos inválidos o duplicados.");
                }
        ...
        toast.promise(saveTournamentPromise(), {
            loading: isEditMode ? 'Actualizando torneo...' : 'Creando torneo...',
            success: isEditMode ? '¡Torneo actualizado!' : '¡Torneo creado con éxito!',
