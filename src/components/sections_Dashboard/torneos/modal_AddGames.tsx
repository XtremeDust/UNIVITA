  ...
  const handleSubmit = async () => {
    ...
    const saveGamePromise = async () => {
        ...
        if (!res.ok) {
            ...
            // Interceptamos errores de validación (ej: horario ocupado)
            if (res.status === 422) {
                throw new Error(data.message || "Conflicto en la programación del juego.");
            }
            throw new Error(data.message || `Error al ${isEditMode ? 'actualizar' : 'crear'} el partido`);
        }
        
        return res;
    };

    toast.promise(saveGamePromise(), {
        loading: isEditMode ? 'Reprogramando partido...' : 'Agendando encuentro...',
        success: isEditMode ? '¡Partido actualizado!' : '¡Partido creado exitosamente!',
