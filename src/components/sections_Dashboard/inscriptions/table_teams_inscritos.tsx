...
import { TeamPdfTemplate } from './TeamPDFTeamplate';
...
export default function table_teams_inscritos() {
    ...
    // --- HOOKS: Todos en la parte superior ---
    ...
    const [teamToDownload, setTeamToDownload] = useState<ApiTeamData | null>(null);
    ...
    const handleDownloadPdf = async (team: ApiTeamData) => {
        if (!pdfTemplateRef.current) {
            console.error("No se encontró la referencia del template para PDF.");
            alert("Error: No se pudo generar el documento (referencia no encontrada).");
            return;
        }
        ...
        try {
            ...
            pdf.save(`Inscripcion_${team.disciplina.replace(/\s/g, '-')}_${team.nombre.replace(/\s/g, '-')}.pdf`);

        } catch (error) {
            console.error("Error al generar o descargar el PDF:", error);
            alert("Ocurrió un error al intentar descargar el PDF.");
        } finally {
            setTeamToDownload(null);
        }
    };

    const handleActionClick = async (buttonId: number, entry: ApiTeam) => {
            
        if (buttonId === 1) { 
            console.log('ACCIÓN: Descargar');
            if (!entry.team_id_for_modal) {
                alert("La descarga del formato de equipo no aplica a inscripciones individuales.");
                return;
            }

            try {
                const res = await fetch(`${API_URL}/teams/${entry.team_id_for_modal}`);
                if (!res.ok) throw new Error("No se pudieron cargar los detalles del equipo para la descarga.");
                const teamJson = await res.json();
                
                const fullTeamData: ApiTeamData = {
                    torneo: entry.torneo,
                    nombre: entry.nombre,
                    logo:entry.logo,
                    disciplina: entry.disciplina,
                    categoria: entry.categoria,
                    color: teamJson.data.color || entry.color || '#FFFFFF',
                    captain: {
                        nombre: teamJson.data.captain?.nombre || entry.captain.nombre,
                        telefono: teamJson.data.captain?.telefono || null,
                    },
                    integrantes_data: teamJson.data.integrantes_data || teamJson.data.integrantes || []
                };

                setTeamToDownload(fullTeamData); 

            } catch (error: any) {
                console.error(error);
                alert("Error al cargar los datos para el PDF: " + error.message);
            }
        }
    ...
    useEffect(() => {
        if (teamToDownload) {
            handleDownloadPdf(teamToDownload); 
        }
    }, [teamToDownload]);
