...
export default function table_subscripts() {          
    ...
    const handleConfirmDelete = async () => {
        ...
        if (!res.ok) {
            ...
            throw new Error(errorData.message || "Error al eliminar suscripción");
        }
        return res;
    };
    ...
    return (
        <Modal state={state}>
            <ContainModal className='bg-white w-full max-w-md'>
                <HeaderModal className="flex-none relative" onClose={onClose}>
                    <h2 className="text-xl font-bold text-gray-800 absolute top-0">Detalles del Suscriptor</h2>
                </HeaderModal>
