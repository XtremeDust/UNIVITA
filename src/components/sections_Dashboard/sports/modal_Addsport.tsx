...
const saveSportPromise = async () => {
        ...
        if (!res.ok) {
            ...
            if (res.status === 422 && errorData.errors) {
                ...
                if (errorData.errors.title) backendErrors.title = errorData.errors.title[0];
                if (errorData.errors.descrip) backendErrors.descrip = errorData.errors.descrip[0];
                if (errorData.errors.mode) backendErrors.mode = errorData.errors.mode[0];
                if (errorData.errors.equipment) backendErrors.equipment = errorData.errors.equipment[0];
                if (errorData.errors.logo) backendErrors.img = errorData.errors.logo[0];
                setErrors(backendErrors);
                throw new Error("Verifica los errores en el formulario.");
            }
    ...
    toast.promise(saveSportPromise(), {
        loading: isEditMode ? 'Actualizando...' : 'Guardando...',
        success: isEditMode ? '¡Deporte actualizado!' : '¡Deporte creado!',
        error: (err) => {
            return err.message; 
        }
    })
    .then(() => {
        if (onSportCreated) onSportCreated();
        handleCloseModal();
    })
    .catch((error) => {
        console.warn("Validación fallida o error de servidor:", error);
    })
    .finally(() => {
        setLoading(false);
    });
