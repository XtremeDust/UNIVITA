...
export default function ModalOfertaAcademica({ state, onClose, isEditMode = false }: ModalProps) {
  ...
  const addStudentToList = (user: User) => {
      if (formData.students.some(s => s.cedula === user.cedula)) {
        toast.error(`${user.nombre} ya está en la lista.`);  
        setSearchError('El estudiante ya está en la lista.');
          return;
      }
      if (formData.students.length >= formData.max_students) {
        toast.error('¡Cupos llenos! Aumenta el límite si es necesario.');
  ...
  const handleCreateUser = async () => {
     ...
     const registerPromise = async () => {
        ...
        if (!res.ok) {
            ...
            if(errData.message?.includes('Duplicate entry')) throw new Error('Este usuario ya existe.');
    ...
    toast.promise(registerPromise(), {
        ...
        success: (createdUser) => {
            ...
            return `¡${createdUser.name} registrado y agregado!`;
        },
  ...
  const handleSubmit = async () => {
     if (!formData.sport_id || !formData.coach_user_id) {
        toast.error("Faltan datos obligatorios (Disciplina o Profesor).");
        return;
    }
    ...
    const saveOfferingPromise = async () => {
        ...
        if (!response.ok) {
            ...
            throw new Error(err.message || 'No se pudo guardar la oferta');
        }
        return response;
    };

    await toast.promise(saveOfferingPromise(), {
        loading: isEditMode ? 'Guardando cambios...' : 'Creando oferta académica...',
        success: '¡Oferta guardada exitosamente!',
